...
**这个 Agent 的存在，是两条链路合并后才成立的。**

另外四个 Agent 都是「通读全站 → 输出固定小节」的分析器。这个形态答不了
指向具体内容的问题：用户问「GEO 和 SEO 有什么区别」，路由只能指向官网信息
分析，而它的八个小节里没有一节是「官网对某个概念是怎么讲的」——
**答案就在已抓取的页面里，系统却没有路径把它取出来。**

补上这条路径需要的正是检索能力，而知识库链路已经有了一套实测过的混合检索
（向量 + BM25，RRF 融合）。把它指向站点语料，缺口即被填上：
